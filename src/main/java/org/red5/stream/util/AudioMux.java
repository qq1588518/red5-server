...
public class AudioMux {
    ...
    public short[] popMuxedData(int maxSamplesCount) {
        /*
         * Example: Suppose we have 3 tracks. One mono, one stereo and one with 3 channels (stereo + bass enhancement) All tracks have different number of samples available, just to make
         * the life a little bit complex Actually, this is closer to reality because source A might have x samples per frame while source B might have y Lij = left, track i, sample j Bij =
         * bass, track i, sample j Rij = right, track i, sample j Track1 - stereo+bass: L11 B11 R11 L12 B12 R12 L13 B13 R13 L14 B14 R14 L15 B15 R15 Track2 - mono: L21 L22 L23 L24 Track3 -
         * stereo: L31 R31 L32 R32 L33 R33 L34 R34 L35 R35 L36 R36 Result (I put each sample on separate row): (L11+L21+L31)/3 B11 (R11+R31)/2 (L12+L22+L32)/3 B12 (R12+R32)/2
         * (L13+L23+L33)/3 B13 (R13+R33)/2 (L14+L34)/2 B14 (R14+R34)/2 (L15+L35)/2 B15 (R15+R35)/2 L36 0 R36 Facts: - all tracks MUST have the same sampling rate - resulted stuff will have
         * 6 samples (maximum from all tracks) - resulted stuff will have 3 channels (maximum number of channels from each track) - to combine tracks, we use simple arithmetic mean value
         */
