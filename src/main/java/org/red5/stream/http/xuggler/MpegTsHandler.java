...
public class MpegTsHandler implements IURLProtocolHandler {
    ...
    /*
     * These following methods all wrap the unsafe methods in try {} catch {} blocks to ensure we don't pass an exception back to the native C++ function that calls these.
     */
    public int close() {
        log.debug("Close {}", url);
        int retval = -1;
    ...
    private Message.Type parsePacket(ByteBuffer in) {
        ...
        if (sync == 0x47) {
            ...
            log.trace("Packet info - scrambling: 0x{} adaptation: {} payload: {} continuity counter: {}", new Object[] { Integer.toHexString(scramblingControl), hasAdaptationField, hasPayloadData, continuityCount });
